...
export function HeroSection() {
  return (
    <section className="relative pt-32 pb-32 px-4 sm:px-6 lg:px-8 bg-white overflow-hidden">
      {/* Éléments décoratifs subtils - Masqués sur mobile */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-[#2563EB]/10 rounded-full blur-3xl opacity-30 -z-0 animate-float hidden md:block"></div>
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-[#16A34A]/10 rounded-full blur-3xl opacity-30 -z-0 animate-float hidden md:block" style={{ animationDelay: "1s" }}></div>
      ...
      <div className="container mx-auto max-w-7xl text-center relative z-10">
        ...
        <ScrollAnimation animation="fadeInUp" delay={100}>
          <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black text-[#0F172A] mb-8 leading-[1.1] tracking-[-0.03em]">
            Vous avez travaillé,
            <br />
        ...
        <ScrollAnimation animation="fadeInUp" delay={300}>
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center mb-12 sm:mb-16">
            <Link href="/login" className="w-full sm:w-auto">
              <Button size="lg" className="w-full sm:w-auto text-base sm:text-lg px-6 sm:px-10 py-5 sm:py-7 bg-[#16A34A] text-white hover:bg-[#16A34A]/90 border-2 border-[#16A34A] rounded-none font-semibold transition-all hover:scale-105 shadow-lg">
                Lancer mon recouvrement
                <ArrowRight className="ml-2 h-4 w-4 sm:h-5 sm:w-5" />
              </Button>
            </Link>
            <Link href="/simulateur" className="w-full sm:w-auto">
              <Button variant="outline" size="lg" className="w-full sm:w-auto text-base sm:text-lg px-6 sm:px-10 py-5 sm:py-7 border-2 border-[#16A34A] text-[#16A34A] hover:bg-[#16A34A] hover:text-white rounded-none font-semibold transition-all">
                Calculer mes économies
              </Button>
            </Link>
            <Link href="#processus" className="w-full sm:w-auto hidden sm:block">
              <Button variant="outline" size="lg" className="w-full sm:w-auto text-base sm:text-lg px-6 sm:px-10 py-5 sm:py-7 border-2 border-[#0F172A] text-[#0F172A] hover:bg-[#0F172A] hover:text-white rounded-none font-semibold transition-all">
